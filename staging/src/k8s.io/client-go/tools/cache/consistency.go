...
// RegisterStore registers a LastSyncRVGetter for a given resource.
func (c *RealConsistencyStore) RegisterStore(resource schema.GroupResource, store LastSyncRVGetter) {
	c.writesLock.Lock()
	defer c.writesLock.Unlock()
	if c.stores == nil {
		c.stores = make(map[schema.GroupResource]LastSyncRVGetter)
	}
	c.stores[resource] = store
}
