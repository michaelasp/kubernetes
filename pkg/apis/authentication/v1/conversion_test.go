...
import (
	...
	extv1 "k8s.io/api/authentication/v1"
	"k8s.io/apimachinery/pkg/api/apitesting/fuzzer"
	metafuzzer "k8s.io/apimachinery/pkg/apis/meta/fuzzer"
	"k8s.io/kubernetes/pkg/api/legacyscheme"
	internalapi "k8s.io/kubernetes/pkg/apis/authentication"
	internalfuzzer "k8s.io/kubernetes/pkg/apis/authentication/fuzzer"
)
...
func TestTokenReviewMemoryIdenticalConversion(t *testing.T) {
	...
	t.Run("v1 to internal", func(t *testing.T) {
		in := &extv1.TokenReview{}
		f.Fill(in)
		out := &internalapi.TokenReview{}
		if err := legacyscheme.Scheme.Convert(in, out, nil); err != nil {
			t.Fatalf("conversion failed: %v", err)
		}
		assertMemoryIdentical(t, "TokenReview", reflect.ValueOf(in).Elem(), reflect.ValueOf(out).Elem())
	})
	t.Run("internal to v1", func(t *testing.T) {
		in := &internalapi.TokenReview{}
		f.Fill(in)
		out := &extv1.TokenReview{}
		if err := legacyscheme.Scheme.Convert(in, out, nil); err != nil {
			t.Fatalf("conversion failed: %v", err)
		}
		assertMemoryIdentical(t, "TokenReview", reflect.ValueOf(in).Elem(), reflect.ValueOf(out).Elem())
...
func TestSelfSubjectReviewMemoryIdenticalConversion(t *testing.T) {
	f := fuzzer.FuzzerFor(fuzzer.MergeFuzzerFuncs(metafuzzer.Funcs, internalfuzzer.Funcs), rand.NewSource(1), legacyscheme.Codecs).NilChance(0).NumElements(1, 1)
	t.Run("v1 to internal", func(t *testing.T) {
		in := &extv1.SelfSubjectReview{}
		f.Fill(in)
		out := &internalapi.SelfSubjectReview{}
		if err := legacyscheme.Scheme.Convert(in, out, nil); err != nil {
			t.Fatalf("conversion failed: %v", err)
		}
		assertMemoryIdentical(t, "SelfSubjectReview", reflect.ValueOf(in).Elem(), reflect.ValueOf(out).Elem())
	})
	t.Run("internal to v1", func(t *testing.T) {
		in := &internalapi.SelfSubjectReview{}
		f.Fill(in)
		out := &extv1.SelfSubjectReview{}
		if err := legacyscheme.Scheme.Convert(in, out, nil); err != nil {
			t.Fatalf("conversion failed: %v", err)
		}
		assertMemoryIdentical(t, "SelfSubjectReview", reflect.ValueOf(in).Elem(), reflect.ValueOf(out).Elem())
